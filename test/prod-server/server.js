...
const app = express();
const PORT = process.env.PORT || 8992; // Prod server port
const SERVER_TYPE = 'production';
...
function createVSXExtensionRaw(extension) {
  ...
  return {
    ...
    preRelease: false, // Production only has stable releases
    ...
    preview: false,
    verified: true, // Production extensions are verified
...
// Helper function to find extensions by ID
function findExtensions(id) {
  return mockData.extensions.filter(ext => ext.id === id);
}

// Helper function to get latest version
function findLatestCompatibleVersion(extensions) {
...
app.get('/plugin/query', (req, res) => {
  ...
  const extension = findLatestCompatibleVersion(findExtensions(id));
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Production Marketplace Server running on http://localhost:${PORT}`);
  console.log(`📊 Server type: ${SERVER_TYPE}`);
  console.log(`📊 Loaded ${mockData.extensions.length} release extensions`);
