...
import { CustomMarketplaceAPI, MarketplaceConfig } from './custom-marketplace-api';
import { VersionValidator } from './version-validator';

const tmpName = promisify(tmp.tmpName);

/**
 * Get custom marketplace configuration from package.json or environment
 */
function getCustomMarketplaceConfig(): MarketplaceConfig {
	// Built-in server addresses (can be overridden by environment variables)
...
/**
 * Validate version against custom marketplace
 */
async function validateVersionWithCustomMarketplace(
	manifest: ManifestPackage,
	options: IPublishOptions
): Promise<void> {
	const marketplaceConfig = getCustomMarketplaceConfig();

	const releaseTag = options.releaseTag || 'release';
	const extensionId = `${manifest.publisher}.${manifest.name}`;
	const versionToValidate = options.version || manifest.version;
	
	log.info(`🔍 Validating version ${versionToValidate} against custom marketplace (${releaseTag})...`);
	
	try {
		const api = new CustomMarketplaceAPI(marketplaceConfig);
		const validator = new VersionValidator(api);

		const validationResult = await validator.validateVersion({
			extensionId,
			localVersion: versionToValidate,
			releaseTag,
			allowEqual: options.skipDuplicate
		});

		VersionValidator.displayValidationResult(validationResult);

		if (!validationResult.isValid) {
			if (options.autoIncrement) {
				throw new Error('Version validation failed even with auto-increment. Please review the validation result above.');
			} else {
				log.info('\n💡 You can use --auto-increment to automatically fix version conflicts.');
				throw new Error('Version validation failed. Use --auto-increment or manually update your version.');
			}
		}

	} catch (error) {
		if (error instanceof Error && error.message.includes('Version validation failed')) {
			throw error; // Re-throw validation errors
		}
		log.warn(`⚠️  Custom marketplace validation failed: ${error instanceof Error ? error.message : String(error)}`);
		log.info('Falling back to Microsoft marketplace validation');
	}
}
...
export interface IPublishOptions {
	...
	readonly releaseTag?: 'release' | 'alpha' | 'beta';
	readonly autoIncrement?: 'patch' | 'minor' | 'major' | 'prerelease';
...
export async function publish(options: IPublishOptions = {}): Promise<any> {
	if (options.packagePath) {
		...
		// Handle auto-increment and version validation
		let updatedOptions = { ...options };
		
		if (options.autoIncrement) {
			const releaseTag = options.releaseTag || 'release';
			const newVersion = VersionValidator.bumpVersion(
				manifest.version,
				options.autoIncrement,
				releaseTag !== 'release' ? releaseTag : undefined
			);
			updatedOptions = { ...options, version: newVersion };
			log.info(`📈 Auto-incremented version: ${manifest.version} → ${newVersion}`);
		}

		// Validate version with custom marketplace if configured
		await validateVersionWithCustomMarketplace(manifest, updatedOptions);

		await prepublish(cwd, manifest, options.useYarn);
		await versionBump(updatedOptions);
